...
-- honest — Sandy's S1 calibrated for ^DJI doesn't transfer to TSLA without
...
-- tickers (e.g. "mean-reversion", "5m", "sandy-jadeja") without forcing a
-- single hierarchy.
--
...
-- Backfill existing strategies (sandy-s1-long was tuned for ^DJI).
UPDATE public.ticket_logics
SET ticker = '^DJI',
    tags = ARRAY['mean-reversion', '5m', 'sandy-jadeja']
WHERE name = 'sandy-s1-long';

-- Backfill Edmund's watchlist DIA row → sandy-s1-long v2 (preserves
...
-- sandy-s1-long was calibrated on. Other tickers on his watchlist are
